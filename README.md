...
> 💡 Alternatively, just launch the app — it will prompt you to enter the key on first run and save it automatically.
...
```bash
pip install google-api-python-client yt-dlp pygame python-dotenv pystray Pillow
```
...
- Adjust **Work / Short Brk / Long Brk** minute values at the top, press Enter to apply
- Click **▶ Start** to begin the Pomodoro session
- After each work block ends: a stand-up popup appears — dismiss it to start the break timer
- Every 4 sessions triggers a 15-min long break

### Music
- Type a song/artist in the search bar and press **Search** or hit Enter
- Click any result to start streaming it
- Use **⏮ ⏯ ⏭** for playback control; **🔀** to toggle shuffle
...
|---|---|
| `google-api-python-client` | YouTube Data API v3 search |
| `yt-dlp` | Extract audio stream URLs from YouTube |
| `pygame` | Audio playback + beep sound |
| `python-dotenv` | Load `.env` API key |
| `pystray` | System tray icon |
| `Pillow` | Generate tray icon image |
...
- YouTube API has a daily free quota of **10,000 units** (each search = 100 units → 100 free searches/day)
- Audio is **streamed**, not downloaded — no files saved to disk
- Tested on **Windows 10/11**, Python 3.10+
